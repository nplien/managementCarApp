...
import {DELIVERY_LIST} from 'configs/StatusConfig';
...
import {IFilterDeliveryState, setStatusDelivery} from '../redux';
...
class StatusDelivery extends Component<IProps, IState> {
  constructor(props: IProps) {
    super(props);
    this.state = {
      arrStatus: DELIVERY_LIST,
  ...
  selectStatus = (status: any) => {
    try {
      ...
      Utilities.logException('StatusDelivery', error);
    }
  };
...
const styles = StyleSheet.create({
  myviewLH: {
    flexWrap: 'wrap',
    flexDirection: 'row',
    ...setPadding(MY_SIZE.s_12, MY_SIZE.s_12, MY_SIZE.s_8, MY_SIZE.s_8),
...
const mapStateToProps = (state: RootState) => {
  let {arrCurrentStatus} = state.FilterDeliveryReducer;
  return {arrCurrentStatus};
};
const mapDispatchToProps = (dispatch: any) => {
  return bindActionCreators({setStatusDelivery}, dispatch);
};
