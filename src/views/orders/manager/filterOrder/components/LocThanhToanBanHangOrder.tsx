...
import {onChangePTBHOrder, setPaymentMethodOrder, IOrderState} from '../../redux';

interface IProps extends IOrderState {
  setPaymentMethodOrder: typeof setPaymentMethodOrder;
  onChangePTBHOrder: typeof onChangePTBHOrder;
}

class LocThanhToanBanHangOrder extends Component<IProps> {
  arrPTTT = ARR_PT_THANHTOAN;
  arrPTBH = ARR_PT_BAN_HANG;

  onPTTTSelected = (pttt: IPaymentItem) => {
    this.props.setPaymentMethodOrder(pttt);
  };

  onPTBHSelected = (ptbh: IMethodSales) => {
    this.props.onChangePTBHOrder(ptbh);
  };
...
const mapPropsToState = (rootState: RootState) => {
  const {arrPTTTDaChon, arrPTBHDaChon} = rootState.OrderReducer;
...
export default connect(mapPropsToState, mapDispatchToProps)(LocThanhToanBanHangOrder);
