...
import {IDashboardState, IStoreAndColor, onCheckStoreReportPieChart} from 'views/dashboard/redux';

interface IProps extends IDashboardState {
  onCheckStoreReportPieChart: typeof onCheckStoreReportPieChart;
}
  ...
  isSelectStore(store: IStoreAndColor) {
    if (this.state.arrStoreUnCheck.length === 0) {
      this.setState(
        ...
        () => {
          this.props.onCheckStoreReportPieChart(store.name, store.color);
        }
      );
    } else {
      ...
      if (indexRemove > -1) {
        ...
        this.props.onCheckStoreReportPieChart(arrRemove[0].name, arrRemove[0].color);
        this.setState({
...
const mapStateToProps = (state: RootState) => {
  const {arrStoreAndColorPieChart, isFirstLoading, isError} = state.DashboardReducer;
