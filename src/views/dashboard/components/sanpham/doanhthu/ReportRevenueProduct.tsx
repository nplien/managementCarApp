...
class ReportRevenueProduct extends Component<IProps> {
    ...
    return (
      <MyView key={item.product_sku + '' + index} transparent>
        <MyView style={styles.content} transparent>
          ...
          <MyText style={styles.textPrice}>{Utilities.convertCount(item.total_value_2)}</MyText>
        </MyView>
  ...
  render() {
    const {arrProductReportByRevenue, isTop10ForSale} = this.props;
    if (isTop10ForSale) return <MyLoading />;
    return (
      <MyView style={styles.container}>{arrProductReportByRevenue?.map(this.renderItem)}</MyView>
    );
  }
}
...
const mapStateToProps = (state: RootState) => {
  const {arrProductReportByRevenue, isTop10ForSale} = state.DashboardReducer;
  return {
...
export default connect(mapStateToProps, null, null, {forwardRef: true})(ReportRevenueProduct);
