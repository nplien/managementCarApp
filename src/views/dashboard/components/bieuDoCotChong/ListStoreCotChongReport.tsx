...
import {IDashboardState, IStoreAndColor, onCheckStoreReportStackChart} from 'views/dashboard/redux';

interface IProps extends IDashboardState {
  onCheckStoreReportStackChart: typeof onCheckStoreReportStackChart;
}
  ...
  isSelectStore(store: IStoreAndColor) {
    if (this.state.arrStoreUnCheck.length === 0) {
      this.setState(
        ...
        () => {
          this.props.onCheckStoreReportStackChart(store.name, store.color);
        }
      );
    } else {
      ...
      if (indexRemove > -1) {
        ...
        this.props.onCheckStoreReportStackChart(arrRemove[0].name || '', arrRemove[0].color || '');
        this.setState({
...
const mapStateToProps = (state: RootState) => {
  const {arrStoreAndColorStackChart, isFirstLoading, isError} = state.DashboardReducer;
  return {
