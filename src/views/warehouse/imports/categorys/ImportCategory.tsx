...
import {getImportCateGory, IImportCateReducerState, showLoadmore, showRefresh} from './redux';
import {ICategoryModel} from 'models/Category.Model';
import {CategoryStyles} from './styles/ImportCate.styles';
...
class Categorys extends Component<IProps> {
    ...
    if (isError) {
      return (
        <MyView style={CategoryStyles.emptyCustomer}>
          ...
          <MyButtonText
            onPress={() => this.props.getImportCateGory()}
            title="Tải lại"
  ...
  render() {
    const {arrImportCateReducer, isRefresh} = this.props;
    return (
      <MyView style={CategoryStyles.container}>
        ...
        <FlatList
          ...
          data={arrImportCateReducer}
          extraData={arrImportCateReducer}
...
const mapStateToProps = (state: RootState) => {
  const {isFirstLoading, isRefresh, arrImportCateReducer, isLoadMore, isStop, isError, cate} =
...
const mapDispatchToProps = (dispatch: any) => {
  return bindActionCreators({getImportCateGory, showRefresh, showLoadmore}, dispatch);
};
