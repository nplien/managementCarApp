...
import {EXPORT_LIST} from 'configs/StatusConfig';
...
import {setStatusExport, IExportOrderState} from '../../redux';
...
class StatusExport extends Component<IProps, IState> {
  constructor(props: IProps) {
    super(props);
    this.state = {
      arrStatus: EXPORT_LIST,
  ...
  selectStatus = (status: any) => {
    try {
      ...
      Utilities.logException('StatusExport', error);
    }
  };
...
const mapStateToProps = (state: RootState) => {
  let {arrCurrentStatus} = state.ExportOrderReducer;
  return {arrCurrentStatus};
};
const mapDispatchToProps = (dispatch: any) => {
  return bindActionCreators({setStatusExport}, dispatch);
};
export default connect(mapStateToProps, mapDispatchToProps, null, {forwardRef: true})(StatusExport);
