...
import ItemChuyenKhoan from './ItemChuyenKhoan';
...
class ViewChuyenKhoan extends PureComponent<IProps> {
  getSoTienCanTra = () => {
  ...
  setCheck = () => {
    ...
    if (arrFormPayment) {
      chuyenKhoan = arrFormPayment.filter(payment => {
        return payment.method === PAYMENT_METHOD.CHUYEN_KHOAN;
      });
    }
  ...
  deleteChuyenKhoan = () => {
    this.props.deleteAllItem(PAYMENT_METHOD.CHUYEN_KHOAN);
  };
      ...
      <MyView>
        <MyView style={styles.container}>
          <MyButton style={styles.content} onPress={this.setCheck}>
            <MyIcon
              ...
              size={20}
            />
            <MyText style={styles.title}>Chuyển khoản</MyText>
          </MyButton>
          {chuyenKhoan.length > 0 && this.getSoTienCanTra() > 0 ? (
            <MyButton style={styles.btnAdd} onPress={this.addChuyenKhoan}>
              <MyIcon
                name={'pluscircle'}
                color={COLOR.TEXT.GREEN}
                iconFontType="AntDesign"
        ...
        <MyView>
          {chuyenKhoan?.map((item: IPaymentOrderModel, index: number) => {
            return <ItemChuyenKhoan key={index} itemPayment={item} />;
          })}
        </MyView>
      </MyView>
    );
...
export default connect(mapStateToProps, mapDispatchToProps)(ViewChuyenKhoan);
