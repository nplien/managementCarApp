...
import {onChangePTBHTraHang, setPaymentMethodTraHang, IReturnOrderState} from '../../redux';

interface IProps extends IReturnOrderState {
  setPaymentMethodTraHang: typeof setPaymentMethodTraHang;
  onChangePTBHTraHang: typeof onChangePTBHTraHang;
}

class LocThanhToanTraHang extends Component<IProps> {
  arrPTTT = ARR_PT_THANHTOAN;
  arrPTBH = ARR_PT_BAN_HANG;

  onPTTTSelected = (pttt: IPaymentItem) => {
    this.props.setPaymentMethodTraHang(pttt);
  };

  onPTBHSelected = (ptbh: IMethodSales) => {
    this.props.onChangePTBHTraHang(ptbh);
  };
...
const mapPropsToState = (rootState: RootState) => {
  const {arrPTTTDaChon, arrPTBHDaChon} = rootState.ReturnOrderReducer;
...
export default connect(mapPropsToState, mapDispatchToProps)(LocThanhToanTraHang);
