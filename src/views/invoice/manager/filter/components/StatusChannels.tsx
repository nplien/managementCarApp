...
import {IInvoiceOrderState, setChannelsInvoice} from 'views/invoice/manager/redux';

interface IProps extends Partial<IInvoiceOrderState> {
  setChannelsInvoice: typeof setChannelsInvoice;
}
...
class StatusChannels extends Component<IProps, IState> {
  onPressChannels = (item: any) => {
    ...
    if (channels === item.id) {
      this.props.setChannelsInvoice(null);
    } else {
      this.props.setChannelsInvoice(item.id);
    }
  };
...
const mapStateToProps = (state: RootState) => {
  let {channels} = state.InvoiceOrderReducer;
  return {channels};
};
