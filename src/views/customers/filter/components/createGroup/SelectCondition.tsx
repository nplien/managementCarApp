...
import {LIST_FIELDS} from 'common/Constants';
import {setPadding, COLOR, MY_SIZE} from 'bases/styles/Core';

interface IProps {
  onSelect: (valueObject: ListField) => void;
  id: string;
}
interface AppState {
  isModal: boolean;
...
export default class SelectCondition extends React.Component<IProps, AppState> {
  constructor(props: IProps) {
    super(props);

    this.state = {
      isModal: false,
      arrModal: LIST_FIELDS
    };
  }
  showHideModal = () => {
    this.setState({
  ...
  renderItemSelectCondition = ({item}: {item: ListField}) => {
    const {id} = this.props;
    return (
      <MyButton
        style={{
